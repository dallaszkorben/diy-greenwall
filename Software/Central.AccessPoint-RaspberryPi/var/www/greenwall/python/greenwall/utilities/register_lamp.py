        ...
        with open(self.reportPath, 'r') as fileObject:
            ...
            for line in lines:
                ...
                try:
                    ...
                    self.lampDict[stationId] = {"ip": ip, "timeStamp": timeStamp}

                except Exception as e:
                    continue

    def register(self, dateString, lampId, ip):
        ...
        with self.lockRegister:
            ...
            self.lampDict[lampId] = {"ip": ip, "timeStamp": timeStamp}

            with open(self.reportPath, 'w') as fileObject:

                for key, value in self.lampDict.items():

                    dateString = datetime.fromtimestamp(value['timeStamp']).astimezone().isoformat()

                    fileObject.write("{dateString}{sep}{lampId}{sep}{ip}{sep}\n".format(dateString=dateString, lampId=key,ip=value["ip"], sep=self.separator))
