...
esp_err_t handler_get_configure(httpd_req_t *req) {
  ...
  //
  //The response will look like this in the python:
  //{
  //  '_content': b"{
  //      'status': 'OK',
  //      'value': {
  ...
  //        'camQuality': 'SXGA',
  //        'camRotate': '1',
  //        'intervalFrameMillis': '30000',
  //        'clientIp': '192.168.0.104',
  //        'clientPort': '80'
  //       }
  //    }",
  //    'status_code': 200,
  //    'headers': {'Content-Type': 'text/html', 'Content-Length': '183'},
  //    'url': 'http://192.168.0.26:80/configure',
  //    'reason': 'OK',
  //}
  //
  return res;
...
esp_err_t handler_post_configure(httpd_req_t *req) {
  ...
  if (error) {
    ...
    strcpy(resp, "{\"status\": \"ERROR\", \"message\": \"Wrong parameter was sent\"}");
  } else {
    ...
    const char* myIntervalFrame = doc["intervalFrameMillis"];
    if (myIntervalFrame) {
